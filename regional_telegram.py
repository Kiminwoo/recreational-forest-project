...
class RegionalTelegramSender:
    ...
    def format_facility(self, facility, max_bytes=3800):
    ...
    def chunk_by_facilities(self, header, facilities, max_chunk_size=4000):
        """시설 단위로 청크 분할"""
        chunks = []
        ...
        for facility in facilities:
            fac_str = self.format_facility(facility)
            fac_bytes = len(fac_str.encode('utf-8'))

            if current_size + fac_bytes > max_chunk_size:
    ...
    def _send_with_retry(self, chat_id, message, region_name, retries=3):
        """재시도 로직이 포함된 전송 함수"""
        ...
        for attempt in range(retries):
            try:
                ...
                if response.status_code == 200:
                    print(f"✅ {region_name} - 전송 성공 (시도 {attempt + 1})")
                    return True
                else:
                    print(f"⚠️ {region_name} - 시도 {attempt + 1} 실패: {response.status_code} - {response.text}")
                    time.sleep(2 ** attempt)  # 지수 백오프
            except Exception as e:
                print(f"⚠️ {region_name} - 시도 {attempt + 1} 예외: {str(e)}")
                time.sleep(2 ** attempt)

        print(f"❌ {region_name} - 최종 전송 실패")
        return False
